...
public class DelOutboundPackageQueueController extends BaseController{
    ...
    * 删除出库单核重记录模块
    */
    @PreAuthorize("@ss.hasPermi('DelOutboundPackageQueue:DelOutboundPackageQueue:remove')")
    @Log(title = "出库单核重记录模块", businessType = BusinessType.DELETE)
    @DeleteMapping("remove")
    @ApiOperation(value = "删除出库单核重记录模块",notes = "删除出库单核重记录模块")
    public R remove(@RequestBody List<String> ids)
    {
    return toOk(delOutboundPackageQueueService.deleteDelOutboundPackageQueueByIds(ids));
    }
